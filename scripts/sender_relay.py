...
def main():
    ...
    id_pub = rospy.Publisher('multibot_relay/robot_id',
                             std_msgs.msg.String, queue_size=1)
    signal_pub = rospy.Publisher(
        'multibot_relay/signal', MultiBotSignal, queue_size=1)
    talking_pub = rospy.Publisher(
        'multibot_relay/talking', MultiBotTalking, queue_size=1)
    ...
    signal_topic = rospy.Subscriber(
        'multibot_sender/signal', MultiBotSignal, signal_callback, queue_size=10)
    talking_topic = rospy.Subscriber(
        'multibot_sender/talking', MultiBotTalking, talking_callback, queue_size=10)
